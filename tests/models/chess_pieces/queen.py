...
QUEEN_POSITION_ROW = 4
QUEEN_POSITION_COL = 4
...
def setup_test():
    ...
    white_piece = Queen(color='white', start_position='low')
    white_knight = Knight(color='white', start_position='low')
    board.get_square(QUEEN_POSITION_ROW, QUEEN_POSITION_COL).piece = white_piece
    board.get_square(3, 3).piece = white_knight
...
def test_piece_moves():
    board = setup_test()
    board.status()
    test_piece = board.get_square(QUEEN_POSITION_ROW, QUEEN_POSITION_COL).piece
    current_position = f"{QUEEN_POSITION_ROW}:{QUEEN_POSITION_COL}"
